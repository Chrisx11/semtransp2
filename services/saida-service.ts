...
/**
 * Atualiza os nomes dos produtos nas saídas com base nos produtos atuais.
 * Produtos que foram deletados não são atualizados (mantém o nome antigo).
 * @returns Número de registros atualizados
 */
export async function atualizarNomesProdutosSaidasSupabase(): Promise<number> {
  const { getProdutosSupabase } = await import("./produto-service")
  
  // Buscar todos os produtos ativos
  const produtos = await getProdutosSupabase()
  
  // Criar um mapa de produtoId -> descricao e categoria
  const produtosMap = new Map<string, { descricao: string; categoria: string }>()
  produtos.forEach((produto) => {
    produtosMap.set(produto.id, { descricao: produto.descricao, categoria: produto.categoria })
  })
  ...
  // Atualizar cada saída se o produto ainda existir
  for (const saida of saidas) {
    const produtoInfo = produtosMap.get(saida.produtoId)
    
    // Se o produto existe e o nome ou categoria é diferente, atualizar
    if (produtoInfo) {
      const precisaAtualizar = 
        produtoInfo.descricao !== saida.produtoNome || 
        produtoInfo.categoria !== saida.categoria
      
      if (precisaAtualizar) {
        try {
          await updateSaidaSupabase(saida.id, {
            produtoNome: produtoInfo.descricao,
            categoria: produtoInfo.categoria,
          })
          atualizados++
        } catch (error) {
          console.error(`Erro ao atualizar saída ${saida.id}:`, error)
        }
      }
    }
  }
