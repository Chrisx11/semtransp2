...
export async function addEntradaSupabase(entrada: Omit<Entrada, "id" | "createdAt">): Promise<Entrada> {
  ...
  const newEntrada = {
    ...
    produtoDescricao: entrada.produtoDescricao,
    responsavelId: entrada.responsavelId,
    responsavelNome: entrada.responsavelNome,
    quantidade: entrada.quantidade,
    valor_unitario: entrada.valorUnitario ?? null,
    data: entrada.data,
    createdAt: now,
  }
...
/**
 * Atualiza os nomes dos produtos nas entradas com base nos produtos atuais.
 * Produtos que foram deletados não são atualizados (mantém o nome antigo).
 * @returns Número de registros atualizados
 */
export async function atualizarNomesProdutosEntradasSupabase(): Promise<number> {
  const { getProdutosSupabase } = await import("./produto-service")
  
  // Buscar todos os produtos ativos
  const produtos = await getProdutosSupabase()
  
  // Criar um mapa de produtoId -> descricao
  const produtosMap = new Map<string, string>()
  produtos.forEach((produto) => {
    produtosMap.set(produto.id, produto.descricao)
  })
  
  // Buscar todas as entradas
  const entradas = await getEntradasSupabase()
  
  let atualizados = 0
  
  // Atualizar cada entrada se o produto ainda existir
  for (const entrada of entradas) {
    const novaDescricao = produtosMap.get(entrada.produtoId)
    
    // Se o produto existe e o nome é diferente, atualizar
    if (novaDescricao && novaDescricao !== entrada.produtoDescricao) {
      try {
        await updateEntradaSupabase(entrada.id, {
          produtoDescricao: novaDescricao,
        })
        atualizados++
      } catch (error) {
        console.error(`Erro ao atualizar entrada ${entrada.id}:`, error)
      }
    }
  }
