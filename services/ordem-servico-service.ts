...
export async function updateOrdemServicoSupabase(
  id: string, 
  dataUpdate: Partial<Omit<OrdemServico, "id" | "numero" | "createdAt" | "updatedAt" | "historico">>, 
  observacao?: string,
  usuarioId?: string,
  usuarioNome?: string
  ...
  // Se mudou o status, adiciona evento ao histórico com o usuário
  ...
  } else if (usuarioId && usuarioNome && Object.keys(dataUpdate).length > 0) {
    // Se não mudou status mas houve atualização e temos usuário, registrar no histórico
    historico = [
      ...historico,
      {
        id: crypto.randomUUID(),
        ...
        tipo: "Atualização",
        de: ordemAtual.status,
        para: ordemAtual.status,
        status: ordemAtual.status,
        observacao: observacao || "Ordem de serviço atualizada",
        usuarioId: usuarioId,
        usuarioNome: usuarioNome,
      },
    ]
...
export async function enviarParaAlmoxarifadoSupabase(
  id: string, 
  observacoes: string,
  usuarioId?: string,
  usuarioNome?: string
...
export async function enviarParaComprasSupabase(
  id: string, 
  observacoes: string,
  usuarioId?: string,
  usuarioNome?: string
