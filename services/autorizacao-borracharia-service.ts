...
export async function updateAutorizacaoBorracharia(
  ...
  try {
    // Converter datas se necessário e aplicar o ajuste de +1 dia (mesmo que na criação)
    const updateData: any = appToDB(autorizacao)
    
    // Processar dataAutorizacao se fornecida
    if (autorizacao.dataAutorizacao !== undefined) {
      // Aplicar o mesmo ajuste de +1 dia usado na criação
      const corrDataAut = addOneDay(autorizacao.dataAutorizacao)
      updateData.data_autorizacao = corrDataAut instanceof Date
        ? corrDataAut.toISOString().split('T')[0]
        : corrDataAut
    }
    
    // Processar dataPrevista se fornecida
    if (autorizacao.dataPrevista !== undefined) {
      // Aplicar o mesmo ajuste de +1 dia usado na criação
      const corrDataPrev = addOneDay(autorizacao.dataPrevista)
      updateData.data_prevista = corrDataPrev instanceof Date
        ? corrDataPrev.toISOString().split('T')[0]
        : corrDataPrev
    }
