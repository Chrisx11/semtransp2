...
import { 
  getAutorizacaoLavadorById, 
  createAutorizacaoLavador, 
...
interface AutorizacaoLavadorDialogProps {
  open: boolean
...
export function AutorizacaoLavadorDialog({
  open,
  onOpenChange,
  editingId,
  viewingId,
  onSuccess,
}: AutorizacaoLavadorDialogProps) {
  ...
  const loadAutorizacao = async () => {
    ...
    try {
      ...
      const autorizacao = await getAutorizacaoLavadorById(editingId || viewingId || "")
      if (autorizacao) {
        ...
        form.reset({
          ...
          // Usar undefined em vez de "" quando não houver lavador para evitar erro de UUID
          lavadorId: autorizacao.lavadorId || undefined,
  ...
  const onSubmit = async (data: AutorizacaoFormValues) => {
    ...
    try {
      if (isEditing) {
        await updateAutorizacaoLavador(editingId!, {
        ...
        await createAutorizacaoLavador({
          veiculoId: data.veiculoId,
          veiculoPlaca: data.veiculoPlaca || "",
          veiculoModelo: data.veiculoModelo || "",
          veiculoMarca: data.veiculoMarca || "",
