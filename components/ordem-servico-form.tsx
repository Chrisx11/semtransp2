...
export function OrdemServicoForm({ onSuccess, onCancel, ordemExistente }: OrdemServicoFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      status: ordemExistente?.status || "Em Aberto",
    ...
    try {
      ...
      try {
        ...
        if (isEdicao && ordemExistente) {
          ...
          const resultado = await updateOrdemServicoSupabase(ordemExistente.id, {
            ...formData,
          ...
          const novaOrdem = await addOrdemServicoSupabase(novaOrdemPayload)
